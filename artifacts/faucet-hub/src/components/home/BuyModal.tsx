...
async function waitForReceipt(
  ...
  return null; // timeout after ~3 min
}
...
export function BuyModal({ chain, onClose }: BuyModalProps) {
  ...
  // Animated dots for confirming step
  useEffect(() => {
  ...
  useEffect(() => {
    if (!chain) {
      ...
      setMainnetTxHash(""); setErrorMsg(""); setSelectedNetwork(null); setConfirmDots(0);
    }
  }, [chain]);
  ...
  const connectWallet = async () => {
    if (!window.ethereum) { setErrorMsg("MetaMask or a Web3 wallet is required."); return; }
    setConnecting(true);
    try {
      const accounts: string[] = await window.ethereum.request({ method: "eth_requestAccounts" });
      if (accounts[0]) setWalletAddress(accounts[0]);
    } catch (err: any) { setErrorMsg(err?.message || "Failed to connect wallet"); }
  ...
  const sendEth = async () => {
    ...
    } catch { /* ignore — wallet may not support switching or already on correct chain */ }
    ...
    // Wait for on-chain confirmation, then auto-submit
    ...
    if (abort.signal.aborted) return; // modal was closed

    if (result === null) {
      // Timeout — show error with manual fallback
    ...
    // Confirmed! Now submit to backend
    setStep("submitting");
    doSubmitBuy(txHash);
  };
        ...
        <div className="px-5 py-5 flex flex-col gap-4">
          ...
          {step === "sending" && (
            <div className="flex flex-col items-center gap-4 py-10 text-center">
              ...
              <div>
                ...
                <p className="text-sm font-mono mt-1" style={{ color: "rgba(255,255,255,0.4)" }}>Confirm the transaction in your wallet</p>
              </div>
            </div>
          )}
          ...
          {step === "error" && (
            <div className="flex flex-col gap-4">
              ...
              {/* Manual fallback — only shown on error/timeout */}
              {mainnetTxHash && (
