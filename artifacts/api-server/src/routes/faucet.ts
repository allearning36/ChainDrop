...
// ── VAST XML proxy ─────────────────────────────────────────────────────────────
// Fetches raw VAST XML server-side and returns it to the browser.
// The browser (VastPlayer.tsx) does all parsing — this proxy exists solely to
// bypass CORS restrictions on ad-network VAST tag URLs.
// Does NOT resolve wrappers server-side; the browser handles the full chain.

router.get("/vast/resolve-xml", async (req, res): Promise<void> => {
  ...
  try {
    ...
    if (!xml.trim()) {
      res.status(502).json({ error: "Empty response from ad server" });
      return;
    }
    // Return raw XML — browser VastPlayer will parse it
